...
class UAVEnv(gym.Env):
    ...
    render_path: (bool) if true, the UAV's path is drawn
    render_shade: (bool) if true, the UAV's shade is drawn
    shade_distance: (int) distance between consecutive UAV's shades
    n_steps: (int) number of time steps
    n_fall_steps: (int) the number of initial steps for which the UAV can't do anything
    change_target: (bool) if true, mouse click change target positions
    initial_throw: (bool) if true, the UAV is initially thrown with random force
    """
    ...
    def __init__(self, render_sim=False, render_path=True, render_shade=True, shade_distance=70,
        ...
        self.UAV_shade_distance = shade_distance
    ...
    def init_pymunk(self):
        ...
        #Generating UAV's starting position
        random_x = random.uniform(200, 600)
        random_y = random.uniform(200, 600)
        angle_rand = random.uniform(-np.pi/4, np.pi/4)
        self.UAV = UAV(random_x, random_y, angle_rand, 20, 100, 0.2, 0.4, 0.4, self.space)

        self.UAV_radius = self.UAV.UAV_radius

    def step(self, action):
        if self.first_step is True:
            if self.render_sim is True and self.render_path is True: self.add_postion_to_drop_path()
            if self.render_sim is True and self.render_shade is True: self.add_UAV_shade()
        ...
        self.UAV.frame_shape.body.apply_force_at_local_point(Vec2d(0, self.left_force), (-self.UAV_radius, 0))
        self.UAV.frame_shape.body.apply_force_at_local_point(Vec2d(0, self.right_force), (self.UAV_radius, 0))

        self.space.step(1.0/60)
        self.current_time_step += 1

        #Saving UAV's position for drawing
        if self.first_step is True:
            if self.render_sim is True and self.render_path is True: self.add_postion_to_drop_path()
        ...
        if self.render_sim is True and self.render_shade is True:
            x, y = self.UAV.frame_shape.body.position
            if np.abs(self.shade_x-x) > self.UAV_shade_distance or np.abs(self.shade_y-y) > self.UAV_shade_distance:
                self.add_UAV_shade()
        ...
        #Stops episode, when UAV is out of range or overlaps
    ...
    def get_observation(self):
        velocity_x, velocity_y = self.UAV.frame_shape.body.velocity_at_local_point((0, 0))
        velocity_x = np.clip(velocity_x/1330, -1, 1)
        velocity_y = np.clip(velocity_y/1330, -1, 1)

        omega = self.UAV.frame_shape.body.angular_velocity
        omega = np.clip(omega/11.7, -1, 1)

        alpha = self.UAV.frame_shape.body.angle
    ...
    def render(self, mode='human', close=False):
        ...
        if len(self.path_UAV_shade):
            for shade in self.path_UAV_shade:
        ...
        vector_scale = 0.05
        l_x_1, l_y_1 = self.UAV.frame_shape.body.local_to_world((-self.UAV_radius, 0))
        l_x_2, l_y_2 = self.UAV.frame_shape.body.local_to_world((-self.UAV_radius, self.froce_scale*vector_scale))
        pygame.draw.line(self.screen, (179,179,179), (l_x_1, 800-l_y_1), (l_x_2, 800-l_y_2), 4)

        l_x_2, l_y_2 = self.UAV.frame_shape.body.local_to_world((-self.UAV_radius, self.left_force*vector_scale))
        pygame.draw.line(self.screen, (255,0,0), (l_x_1, 800-l_y_1), (l_x_2, 800-l_y_2), 4)

        r_x_1, r_y_1 = self.UAV.frame_shape.body.local_to_world((self.UAV_radius, 0))
        r_x_2, r_y_2 = self.UAV.frame_shape.body.local_to_world((self.UAV_radius, self.froce_scale*vector_scale))
        pygame.draw.line(self.screen, (179,179,179), (r_x_1, 800-r_y_1), (r_x_2, 800-r_y_2), 4)

        r_x_2, r_y_2 = self.UAV.frame_shape.body.local_to_world((self.UAV_radius, self.right_force*vector_scale))
    ...
    def reset(self):
        self.__init__(self.render_sim, self.render_path, self.render_shade, self.UAV_shade_distance,
    ...
    def initial_movement(self):
        if self.initial_throw is True:
            ...
            self.UAV.frame_shape.body.apply_force_at_world_point(throw, self.UAV.frame_shape.body.position)

            throw_rotation = random.uniform(-3000, 3000)
            self.UAV.frame_shape.body.apply_force_at_local_point(Vec2d(0, throw_rotation), (-self.UAV_radius, 0))
            self.UAV.frame_shape.body.apply_force_at_local_point(Vec2d(0, -throw_rotation), (self.UAV_radius, 0))
    ...
    def add_UAV_shade(self):
        x, y = self.UAV.frame_shape.body.position
        self.path_UAV_shade.append([x, y, self.UAV.frame_shape.body.angle])
        self.shade_x = x
        self.shade_y = y
